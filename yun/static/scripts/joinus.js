...
$(function() {
    var index = 0,
        ...
        // 获取切换的页面的数量
        time = 1,
        pageLength = document.getElementByClassName('tq-slider-page').length,
    ...
    function getIndexByHash() {
        ...
        if (hash) {
            document.querySelectorAll('.tq-slider-wrap')
              .querySelectorAll('.tq-slider-page')
              .some(function (v, i) {
                if (this.dataset('id') == hash) {
                  index = i;
                  return false;
                }
              });
    ...
    // 切换页面动画开始前的设置
    function slider() {
        $oldPage = document.querySelectorAll('.tq-slider-wrap').querySelectorAll('.page-current');
